<script>
  ...
  import { connected, flashStatus, robotState, mapTabEnabled } from '../lib/stores.js';
...
<div class="flex flex-col h-full bg-[#161920] text-slate-200">
  ...
  <div class="flex-1 flex items-start justify-center p-8 overflow-y-auto">
    ...
    {#if shutdownSent}
      <!-- Sent state -->
      <div class="max-w-md w-full flex flex-col items-center gap-4 text-center py-12">
    ...
    {:else if confirming}
      <!-- Confirmation step -->
      <div class="max-w-md w-full bg-[#1e2129] rounded-xl border border-red-700/40 p-6 space-y-4">
    ...
    {:else}
      <!-- Normal state — two columns: resources/camera on the left, power/firmware on the right -->
      <div class="max-w-4xl w-full grid grid-cols-2 gap-8 items-start">
        <div class="space-y-8">
