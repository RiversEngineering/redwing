...
// Sharp GP2Y0A21YK0F distance formula: D_cm = 27.86 × V_actual^(-1.15)
// With a 10kΩ/10kΩ voltage divider: V_actual = V_adc × 2
//   V_adc  = raw × 3.3 / 4095
//   V_actual = raw × 6.6 / 4095
...
uint16_t ir_distance_read_mm(uint8_t adc_channel, bool *valid) {
    ...
    float v_actual = (float)raw * (6.6f / 4095.0f);
